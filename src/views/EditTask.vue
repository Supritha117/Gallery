...
export default {
    ...
    load() {
        ...
        if(this.form.taskName=='' && this.form.description=='' && this.form.username=='' && this.form.listType==''){
          this.$router.push({
           name: "NF"
         });
        }
    ...
    async save(){
        ...
        console.log(err);
            ...
            this.$swal('Success','Edited successfully',"success")
            this.$router.push({
              name: "TaskList"
            });
          }
      ...
      async addNewTask() {
        ...
        for(var k=0;k<List.length;k++){
          for(var l=0;l<List[k].items.length;l++){
            if(List[k].items[l].taskName==this.form.taskName && List[k].items[l].id!=this.id){
